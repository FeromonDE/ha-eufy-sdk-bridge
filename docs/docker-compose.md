...
| `SNAPSHOT_LIVE`    | `auto`                          | controls `/snapshot/<sn>`: `auto` takes a fresh live still only from **mains-powered** cameras; **battery** cameras use the retained/persisted last-event thumbnail without a radio wake. `1` forces live stills everywhere; `0` disables them everywhere |
